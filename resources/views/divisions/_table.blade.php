    ...
    <tbody>
      @forelse ($divisions as $division)
        <tr>
          ...
          <td>{{ $division->updated_at ? $division->updated_at->format('d M Y') : '-' }}</td>
          <td>{{ $division->updater->name ?? '-' }}</td>
          <td>
            <div class="dropdown text-center">
              ...
              <ul class="dropdown-menu">
                ...
                <li>
                  <form action="{{ route('divisions.destroy', $division->id) }}" method="POST" id="delete-form-{{ $division->id }}">
                    @csrf
                    @method('DELETE')
                    <button type="button" class="dropdown-item text-danger" onclick="confirmDelete({{ $division->id }})">Delete</button>
                  </form>
                </li>
              </ul>
            </div>
